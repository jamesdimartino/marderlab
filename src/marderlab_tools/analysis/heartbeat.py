...
def analyze_experiment(records: list[TraceRecord], settings: PipelineSettings) -> dict[str, Any]:
    output: dict[str, Any] = {"pipeline": "heartbeat", "files": [], "summary": {}, "flags": []}
    rate_groups: dict[str, list[float]] = {}
    rates: list[float] = []
    ...
    for record in sorted(records, key=lambda r: int(r.metadata.get("file_index", 0))):
        ...
        key = str(temp) if temp is not None else "unknown"
        rate = float(metrics.get("heart_rate_bpm", 0.0))
        rate_groups.setdefault(key, []).append(rate)
        rates.append(rate)
        for flag in flags:
    ...
    output["summary"] = {
        ...
        "stats_by_temperature": compute_stat_markers(rate_groups),
    }
    return output
