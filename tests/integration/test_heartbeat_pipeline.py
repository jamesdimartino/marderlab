...
def _write_config(path: Path, raw_root: Path, processed_root: Path, cache_root: Path) -> None:
    payload = {
        ...
        "pipelines": {
            "heartbeat": {
                "metadata_tabs": ["FTSpont"],
                "experiment_type_values": ["heartbeat"],
                "force_channel": "force",
                "trig_channel": "trig",
            }
        },
...
def test_heartbeat_pipeline_writes_outputs(tmp_path: Path, monkeypatch) -> None:
    raw = tmp_path / "raw"
    processed = tmp_path / "processed"
    cache = tmp_path / "cache"
    exp = raw / "997_333"
    exp.mkdir(parents=True)
    (exp / "997_333_0001.abf").write_bytes(b"")
    ...
    def fake_abf_loader(file_path, _channel_map):
        ...
        trigger = np.zeros(n)
        y = 0.1 * np.sin(2 * np.pi * 1.2 * t) + 0.02 * np.sin(2 * np.pi * 2.4 * t)
        return FakeTrace(
            file_path=file_path,
    ...
    report = orchestrator.run_pipeline(config, "heartbeat", generate_plots=False)
    assert report["summary"]["total_experiments"] == 1
    assert report["summary"]["success_count"] == 1
    npy_file = processed / "997_333" / "npy" / "heartbeat_metrics.npy"
    tidy_csv = processed / "997_333" / "npy" / "heartbeat_metrics_tidy.csv"
